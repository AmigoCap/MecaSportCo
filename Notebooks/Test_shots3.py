...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 17 09:51:16 2019
...
def detect_shoot(moments,i,players):
    ...
    if ball_in_tube2(ball,basket_pos):
        k=j
        h=ball[2]
    
        j=i
        while j<(len(moments)-2) and ball[2]>3.05*3.28:
            j+=1
            ball0=ball
            while (not test_moment(moments[j]) or not test_moment(moments[j+1])) and (j<(len(moments)-2)):
                j+=1
        ...
        if not ball_in_tube2(ball0,basket_pos):
            print(k,j)
            print(j,h,ball0[2])
...
def ball_in_tube2(ball,basket_pos):
    if distance(ball[0],basket_pos)<7:  
...
#        ball=players2[0]
#        
#    print(j,ball_in_tube2(ball,basket_pos))
#    
#    j=i
#    while j<(len(moments)-2) and ball[2]>3.05*3.28:
#        j+=1
