...
def old_event_correlation(event,F):
    deb=time.time()
    moments=event['moments']
    T=moments[0][2]-moments[-1][2]
    ...
    for k in range(len(moments)-1):
        ...
        for i in range(50):
            for j in range(94):
                ...
                deltat=sp.time_difference(mom_infos,b,F)
    
                DELTA_X[i][j][k]=deltax
                DELTA_T[i][j][k]=deltat
...
#            
#        
#for i in range(len(frames_forces)):
#    plt.plot(frames_forces[i].R,label=str(i))
#plt.legend()
#    
#TAU=[]
#for k in range(len(forces)):
#    TAU.append(frames_forces[k].iloc[0]['tau'])
#plt.plot(forces,TAU)
#
#RHO=[]
#for k in range(len(forces)):
#    RHO.append(frames_forces[k].rho)
#plt.plot(forces,RHO)
